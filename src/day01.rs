...
pub fn part2<R: BufRead>(mut input: R) -> String {
    ...
    while let Ok(num_bytes_read) = input.read_line(&mut buf) {
...

        {
            ...
            // Actual solution logic is here:
            if letter == 'L' {
                let prev = current_lock_position;
                current_lock_position -= number_of_turns;
                if prev > 0 && current_lock_position <= 0 {
                    result += 1;
                }
                result += (current_lock_position / 100).abs();
                current_lock_position = ((current_lock_position % 100) + 100) % 100;
            } else if letter == 'R' {
                current_lock_position += number_of_turns;
                result += current_lock_position / 100;
                current_lock_position %= 100;
            } else {
                panic!(
                    "Error: first symbol in each of the lines should be either L for the left turn or R for the right turn"
